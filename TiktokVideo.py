...
def GetWebdriver(ini):
    if ini.driver is None:
        print('*****************  初始化浏览器  *********************')
        ini.driver = webdriver.Chrome(executable_path=r'C:\Program Files\Google\Chrome\Application\chromedriver.exe',
                                      desired_capabilities=ini.caps)

        if len(ini.header['cookie']):
            print('*****************  检测存在cookie，尝试加载cookie登录  *********************')
...
# 抓取首页script[crossorigin="anonymous"]的用户数据
def GetFrontPageUserData(ini):
    print('*****************  正在处理首页数据，请勿操作  *********************')
    soup = BeautifulSoup(ini.driver.page_source, 'html.parser')
    return json.loads(json.loads(soup.select('script[crossorigin="anonymous"]')[-1].text[19:-1])[1][18:-2])
...
def Start():
    try:
        ...
        while True:
            print('=====================================================')
            print('*********** 抖音作品爬虫工具 V20231223.2 ***************')
            print('=====================================================')
            print(
                '1.下载用户主页作品(可用)\t\t2.下载用户喜欢作品(可用)\n3.下载用户作品合集(可用)\t\t4.下载自己收藏作品(可用)\n5.批量下载用户作品(可用)\t\t6.退出程序')
            print('=====================================================')
...
def Home(ini, *url):
    ...
    # # 发起线程加载后续数据
    # myThread = threading.Thread(target=ScrollWebdrive, args=(ini,))
    ...
    # 处理首页数据
    data = GetFrontPageUserData(ini)

    # 数据节点的这个数字会变，所以直接查
    # key = [a for a in data if 'uid' in data[a]][0]

    # 获取用户名称
    ini.authorName = ini.replace(data["user"]["user"]["nickname"])
    ...
    if ini.userAvatarDownload:
        userAvatarURL = 'https:' + data["user"]["user"]["avatar300Url"]
...
def Collection(ini, *url):
    ...
    ini.authorName = ini.replace(data["aweme"]['detail']["authorInfo"]["nickname"])
    mix_id = data["aweme"]['detail']["mixInfo"]["mixId"]
    collection_name = ini.replace(data["aweme"]['detail']["mixInfo"]["mixName"])
...
# 下载收藏作品
def Favorite(ini):
    ini.url = 'https://www.douyin.com/user/self'
