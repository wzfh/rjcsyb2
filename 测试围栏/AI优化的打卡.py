...
def 截图():
    ...
    # 获取屏幕尺寸
    left_ratio = 86 / screen_width  # 约 0.08
    top_ratio = 746 / screen_height  # 约 0.39
    right_ratio = 940 / screen_width  # 约 0.87
    bottom_ratio = 1527 / screen_height  # 约 0.79

    # 根据实际屏幕尺寸计算坐标
    left = int(screen_width * left_ratio)
    top = int(screen_height * top_ratio)
    right = int(screen_width * right_ratio)
    bottom = int(screen_height * bottom_ratio)  # 使用完整高度

    if os.path.exists('img/bei.jpg'):
        os.remove('img/bei.jpg')
        print("文件 bei.jpg 已被成功删除.")

    region_image = imge.crop((left, top, right, bottom))
    print('重新截取屏幕')
    region_image.save('img/bei.jpg')
    识别图片2('img/bei.jpg')
    time.sleep(5)
